...
inline void aorb(bool a) {if (a) cout << "Alice" << el; else cout << "Bob" << el;}
// mt19937 rng((int)std::chrono::steady_clock::now().time_since_epoch().count());
constexpr int N = 1e6 + 1; //remember constraints dumbass
constexpr int lim = 31623;
//<------------- Solution --------------->

void dfsg(auto& adj, auto& col, int v, int c) {
  col[v] = c;
  for (int u : adj[v]) {
    if (col[u] == 0) {
      dfsg(adj, col, u, c);
...
int dfsf(auto& adj, auto& col, auto& colg, int v, int c) {
  col[v] = c;
  int res = 0;
  for (int u : adj[v]) {
    if (col[u] == 0) {
      if (colg[u] != c) res++;
      else res += dfsf(adj, col, colg, u, c);
    }
  }
  return res;
}

void read(auto& adj, int m) {
  while (m--) {
    r2(u, v);
    adj[--u].eb(--v);
    adj[v].eb(u);
...
void solve () {
  r3(n, m1, m2);
  vvint adjf(n), adjg(n);
  read(adjf, m1); read(adjg, m2);

  vint colf(n), colg(n);
  int res = 0;
  FOR(i, n) {
    if (colg[i] == 0) {
      dfsg(adjg, colg, i, i + 1);
    }
    if (colf[i] == 0) {
      res += dfsf(adjf, colf, colg, i, colg[i]);
      res += (colg[i] < i + 1);
    }
  }
