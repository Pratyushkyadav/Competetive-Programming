...
void get() {
  p[0] = 1;
  FR(i, 1, N) p[i] = (p[i - 1] * 1ll * 2 )% MOD2;
}
void solve() {
  r1(n);
  int a[n], b[n]; rep(a, n); rep(b, n);
  int x = 0, y = 0;
  FOR(i, n) {
    if (a[i] > a[x]) x = i;
    if (b[i] > b[y]) y = i;
    int res = 0;
    if (a[x] > b[y]) res = (p[a[x]] + p[b[i - x]]) % MOD2;
    else if (a[x] < b[y]) res = (p[a[i - y]] + p[b[y]]) % MOD2;
    else if (b[i - x] > a[i - y]) res = (p[a[x]] + p[b[i - x]]) % MOD2;
    else res = (p[a[i - y]] + p[b[y]]) % MOD2;
    cout << res << ' ';
  }
  cl;
}
