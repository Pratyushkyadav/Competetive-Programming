...
//<------------- Solution --------------->
void dfs(auto& t, auto& lvl, int l, int u, int p) {
  if (l == sz(lvl)) lvl.pb(1);
  else lvl[l]++;
  for (int v : t[u]) {
    if (v != p) dfs(t, lvl, l + 1, v, u); 
  }
}
...
void solve() {
  ...
  vi lvl;
  dfs(t, lvl, 0, 0, -1);
  ll res = 0; int odd = 0, even = 0;
  FOR(i, sz(lvl)) {
    if (i & 1) {
      res += lvl[i] * max(even - 1, 0);
      odd += lvl[i];
    } else {
      res += lvl[i] * max(odd - 1, 0);
      even += lvl[i];
    }
  }
  c1(res);
