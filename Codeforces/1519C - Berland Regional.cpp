...
#define cv(v) for (int i = 0; i < sz(v); ++i) cout << v[i] << " \n"[i + 1 == sz(v)];
#define cl cout << el;
...
constexpr int N = 1e4 + 1; //remember constraints dumbass
constexpr int lim = 31623;
//<------------- Solution --------------->

void solve () {
  r1(n);
  int u[n], s[n]; rep(u, n); rep(s, n);
  vvi a(n);
  FOR(i, n) {
    a[u[i] - 1].pb(s[i]);
  }
  ...
  FOR(i, n) {
    sort(rall(a[i]));
    for (int x : a[i]) p[i].pb(p[i].back() + x);
  }
  vl res(n);
  for (auto& v : p) {
    FR(k, 1, sz(v)) {
      res[k - 1] += v[(sz(v) - 1) / k * k];
    }
  }
  cv(res);
}
