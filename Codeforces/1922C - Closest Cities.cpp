...
constexpr int N = 1e2 + 1; //remember constraints dumbass
constexpr int lim = 31623;
//<------------- Solution --------------->
void solve() {
  r1(n);
  int a[n], l[n], r[n]; rep(a, n);
  l[n - 1] = r[0] = 0;
  auto check = [&](int i) {
    int dl = (i > 0) ? a[i] - a[i - 1] : inf;
    int dr = (i < n - 1) ? a[i + 1] - a[i] : inf;
    return dl < dr;
  };
  FORR(i, n - 2) {
    l[i] = l[i + 1] + (check(i + 1) ? 1 : a[i + 1] - a[i]);
  }
  FR(i, 1, n) {
    r[i] = r[i - 1] + (check(i - 1) ? a[i] - a[i - 1] : 1);
  }
  r1(m);
  while (m--) {
    r2(u, v); --u, --v;
