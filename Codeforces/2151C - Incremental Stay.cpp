...
hash_standard_resize_policy<hash_exponential_size_policy<>, hash_load_check_resize_trigger<true>, true>>;
constexpr void add(int &a, int b, int mod = MOD) { a += b; if(a >= mod) a -= mod; }
...
void solve() {
  r1(n); 
  int a[2 * n]; rep(a, 2 * n);
  ll res[n + 1]; res[0] = res[1] = 0;
  FOR(i, 2 * n) {
    res[!(i & 1)] += a[i], res[i & 1] -= a[i]; 
  }
  res[1] += 2 * 1ll * (a[2 * n - 1] - a[0]);
  FR(i, 2, n) {
    res[i] = res[i - 2] + 2 * 1ll * (a[2 * n - i] - a[i - 1]);
  }
  FOR(i, n) cout << res[i] << ' '; cl;
}
